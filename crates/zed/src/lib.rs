...
pub fn init(cx: &mut App) {
    ...
    // let cx = Application::with_platform(gpui_platform::current_platform(false)).with_assets(Assets);

    let system_id = cx.background_executor().spawn(system_id());
    let installation_id = cx.background_executor().spawn(installation_id());
